...
function getSubjects(str){
	...
	for (var j = 1; j < tr.length; j++){
		...
		if(scodes.indexOf(scode) == -1){
			...
			treecellW.setAttribute("label", "0");
			treecellW.setAttribute("id", "subW"+scode);
...
function writeToFile(data)
{
	...
	data += "\n";	
	data += "Subjects,Passed,Failed,Absent,With-held,Percentage\n"
	for (var j = 0; j < total_sub; j++){
		...
		data += document.getElementById("subW"+scodes[j]).getAttribute("label")+",";
	...
	data += "FCD:"+fcd+", FC:"+fc+", SC:"+sc+", WH:"+wh;
...
function updatePerc(){
	...
	document.getElementById("wh").setAttribute("value", wh);
}
...
function getSubjectsStatus(str){
	...
	for (var j = 1; j < tr.length; j++)
	{
		...
		else if(($(td).eq(4).text()).indexOf("W") > -1){
			v=Number(document.getElementById("subW"+scode).getAttribute("label"));
			v++;
			document.getElementById("subW"+scode).setAttribute("label", v);
		}	
		...
		//alert(pass1+" "+fail1+ ((pass1/(pass1+fail1))*100).toFixed(2));
		if(pass1==0 && fail1==0){
		}
		else
	...
	{		
		...
		{		
			...
			{	
				...
				var grade = replaceUnwantedChar($(table).eq(0).find("tr").eq(0).find('td').eq(3).text());
				
				if(checkForWithHeald(grade)){
					wh++;
					//alert(wh);
...
{	
	...
	whv 	= document.createElement("label");
	whv.setAttribute('value', 'WH:');
	...
	whv1 	= document.createElement("label");
	whv1.setAttribute('value', '0');		
	whv1.setAttribute('id', 'wh');
